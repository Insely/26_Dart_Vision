...
bool Detector::findTarget(const cv::Mat &input, cv::Point2f &best_center)
{
    ...
    /*mask_: 这里使用的是类的成员变量 mask_（通常是 preprocess 函数生成的黑白二值图），而不是传入的 input。

    cv::findContours: OpenCV 的核心函数，用于在黑白图中找白色的连通区域。

    cv::RETR_EXTERNAL: 关键参数。只检测最外围的轮廓。如果目标内部有空洞（比如甜甜圈），空洞会被忽略，这能减少计算量。

    cv::CHAIN_APPROX_SIMPLE: 压缩轮廓点。比如一条直线，它只存起点和终点，不存中间所有的点，节省内存。*/
    ...
    for (const auto &contour : contours)
    {
        ...
        double circularity = (4 * CV_PI * area) / (perimeter * perimeter); // perimeter: 轮廓的周长 circularity: 圆度公式
        /*对于完美的圆，结果是 1.0。
        ...
        /*minAreaRect: 给轮廓套一个最小外接旋转矩形（即兴的矩形，可以斜着框住物体）。

        ratio: 计算长宽比。无论宽大还是高大，永远用 长边 / 短边，确保 ratio >= 1.0。正方形或圆形的 ratio 接近 1.0。*/
        ...
        /*初筛：只看那些“比较圆”（circularity > 0.5）且“不太长”（ratio < 1.8）的轮廓。
        ...
        area / 100.0：面积越大分越高（优先锁定离摄像头近的、大的目标，而不是远处的小噪点）。

        更新最佳结果：如果当前轮廓分数超过了历史最高分，它就暂时成为“最佳目标”，记录它的中心点 min_rect.center。*/
    }
    return frame_found;
}
...
DetectionResult Detector::process(const cv::Mat &frame)
{
    ...
    /*is_locked: 只有当连续看到的次数 found_count_ 超过阈值（min_found_frame_，比如设为 5 帧）时，
    ...
    结果为负数表示目标在画面左边，正数表示在右边。这个值通常直接传给 PID 控制器用来控制机器人的转向。*/
